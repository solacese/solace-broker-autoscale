...
The 23 September follow-up added real local static 2/3/4-broker routing evidence, open-loop multiprocess workload measurements, conservative fanout attribution, and lifecycle crash-recovery hardening. See the [feature qualification matrix](feature-qualification-matrix-2026-09-23.md). These results remain functional and below saturation; automatic sequential 1→4 scale-out evidence is still pending and must not be inferred from the three preassigned static topologies.
