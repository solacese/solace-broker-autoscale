...
The outbox permits one writer process. Restart with the **same file on a persistent local volume**. Each publisher replica needs its own file. Keep that directory private; it contains payloads. A full buffer rejects new work explicitly. Defaults: 100,000 pending records, 100 MB of serialized logical pending records, 1 MiB per payload and 128 concurrent partitions. `MaxOutboxBytes` counts one serialized logical identity record per pending event; the lane copy, ID index, database pages, freelist and file high-water allocation require additional disk space. It is a backpressure threshold, not a filesystem quota. Monitor the database file and provision storage separately. Do not use a network filesystem or delete the outbox to clear an error.
