...
The [22–23 September Cloud qualification](cloud-qualification-2026-09-22.md) found no Enterprise 100K quota. The largest feasible two-service run used Enterprise 5K HA and reconciled 112 accepted events in each of two subscriber groups, with zero duplicates and preserved per-account order through migration and publisher SIGKILL/restart. A separate Enterprise 10K HA run exercised four bounded AMQP-over-TLS payload/fanout/slow-consumer cases with complete reconciliation. Its sequential remote client did not seek saturation, so neither result turns the normalized percentages above into measured 5K, 10K or 100K capacity.
