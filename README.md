...
> customer data - you measure your own brokers (see [`docs/benchmark.md`](docs/benchmark.md)).
...
```bash
# install
pip install -e '.[compile]'      # add the xlsx compiler; use '.[dev]' for tests and linters too

# get a recommendation (runs on the bundled sample model until you add your own)
solace-autoscale recommend --config config.example.yaml --metrics metrics.json

# see what you'd need if traffic grows
solace-autoscale whatif --config config.example.yaml --metrics metrics.json --multipliers 1,2,4
```

Both commands above run against `models/synthetic-v0.json`, a bundled sample with obviously fake
numbers that every report flags and that is **blocked from any real action**. To use your own
numbers, compile a model from a measured workbook (see [`docs/benchmark.md`](docs/benchmark.md)):
...
Then point `capacity.model` in your config at the file you just built.
...
- **Your capacity numbers** are yours to measure - [`docs/benchmark.md`](docs/benchmark.md) explains the
